...
這裡有一個歷史遺留的問題，因為我沒有在一開始就設定多種語言，現有文章的 URL 
並沒有 language code。如果設定 `defaultContentLanguageInSubdir: true`
，那麼所有文章的 URL 都會多出 zh-tw 或 en 等 language code。  
這樣做會重設所有的 URL 對 SEO 很不好，因此我選擇設定為 false，繁體中文的 URL 就不會變化。  
壞處是哪天我想把英文變預設的時候，還是必須面臨一次 SEO 重建期。


# 免責聲明

下面提到我選用的翻譯工具是 OpenAI API，因為是 AI 翻譯還是在文章開頭加上一段免責聲明。  
